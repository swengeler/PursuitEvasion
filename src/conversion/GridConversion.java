...
public class GridConversion {
    ...
    public static int[][] convert(ArrayList<Polygon> polygons, double cellSize) {
    ...
    public static void convert(ArrayList<MapPolygon> polygons, ArrayList<Circle> pursuers, ArrayList<Circle> invaders, double cellSize) {
        double minX = Double.MAX_VALUE, minY = Double.MAX_VALUE, maxX = -Double.MAX_VALUE, maxY = -Double.MAX_VALUE;
        for (int i = 0; i < polygons.get(0).getPoints().size(); i += 2) {
            if (polygons.get(0).getPoints().get(i) < minX) {
                minX = polygons.get(0).getPoints().get(i);
